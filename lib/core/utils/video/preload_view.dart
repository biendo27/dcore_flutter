...
/// [PreloadPageView]s.
class PreloadPageMetrics extends FixedScrollMetrics {
  ...
  double? get page {
    return max(0.0, pixels.clamp(minScrollExtent, maxScrollExtent)) / max(1.0, viewportDimension * viewportFraction);
  }
...
class _PagePosition extends ScrollPositionWithSingleContext implements PreloadPageMetrics {
  _PagePosition({
  ...
  double? getPageFromPixels(double? pixels, double? viewportDimension) {
    ...
    return max(0.0, pixels) / max(1.0, viewportDimension * viewportFraction);
  }
  ...
  void saveScrollOffset() {
    PageStorage.of(context.storageContext).writeState(context.storageContext, getPageFromPixels(hasPixels ? pixels : null, hasViewportDimension ? viewportDimension : null));
  }
...
class PreloadPageScrollPhysics extends ScrollPhysics {
  /// Creates physics for a [PreloadPageView].
  const PreloadPageScrollPhysics({super.parent});

  @override
  PreloadPageScrollPhysics applyTo(ScrollPhysics? ancestor) {
    return PreloadPageScrollPhysics(parent: buildParent(ancestor));
  }
...
const PreloadPageScrollPhysics _kPagePhysics = PreloadPageScrollPhysics();
...
class PreloadPageView extends StatefulWidget {
  ...
  /// [PreloadPageScrollPhysics] prior to being used.
  ///
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (ScrollNotification notification) {
        if (notification.depth == 0 && widget.onPageChanged != null && notification is ScrollUpdateNotification) {
          final PreloadPageMetrics metrics = notification.metrics as PreloadPageMetrics;
